...
def experiment(variant):
    ...
    # FOR EASY IN AIR FETCH LARGER OBJECT RANGE ------------------------------------
    acts_max = Variable(ptu.from_numpy(np.array([0.24999844, 0.24999035, 0.24999848, 0.01499987])), requires_grad=False)
    acts_min = Variable(ptu.from_numpy(np.array([-0.24999948, -0.24999969, -0.24999971, -0.01499985])), requires_grad=False)
    observation_max = Variable(ptu.from_numpy(np.array([0.14981718, 0.14922823, 0.00105448, 0.19316468, 0.20144443,
        0.20205348, 0.05088978, 0.05087405, 0.01012868, 0.01011336])), requires_grad=False)
    observation_min = Variable(ptu.from_numpy(np.array([-1.49439076e-01, -1.49636276e-01, -1.10015137e-01, -1.99832936e-01,
