...
class BaseMap(nn.Module):
    ...
    def __init__(
        ...
        if siamese_input:
            concat_dim = len(all_input_dims) * siamese_input_layer_dim
        else:
            concat_dim = sum(all_input_dims)
    ...
    def forward(z, inputs):
        ...
        if self.siamese_input:
            siamese_input_results = list(
                map(
                    lambda x, seq: seq(x),
                    zip(all_inputs, self.siamese_input_seqs)
                )
            )
            hidden_input = torch.cat(siamese_input_results, dim=1)
        else:
            hidden_input = torch.cat(all_inputs, dim=1)=
